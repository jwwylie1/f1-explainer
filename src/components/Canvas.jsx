...
function Canvas({ race }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [car1LocRes, car2LocRes, car1DataRes, car2DataRes] = await Promise.all([
        fetch(
          "https://api.openf1.org/v1/location?session_key=9126&driver_number=81&date%3E2023-07-09T14:34:16.764000+00:00&date%3C2023-07-09T14:35:49.527000+00:00"
        ),
        fetch(
          "https://api.openf1.org/v1/location?session_key=9693&driver_number=27&date%3E2025-03-16T04:57:43.997000+00:00&date%3C2025-03-16T04:59:17.156000+00:00"
        ),
        fetch (
          "https://api.openf1.org/v1/car_data?session_key=9693&driver_number=81&date%3E2025-03-16T04:56:52.676000+00:00&date%3C2025-03-16T04:58:22.881000+00:00",
        ),
        fetch(
          "https://api.openf1.org/v1/car_data?session_key=9693&driver_number=27&date%3E2025-03-16T04:57:43.997000+00:00&date%3C2025-03-16T04:59:17.156000+00:00"
        )
      ]);
  ...
  useEffect(() => {
    if (car1Location && car2Location && race) {
      ...
      img.onload = () => {
        ...
        setDriver1({x: INIT_X*canvasWidth, y: INIT_Y*canvasHeight})
        setDriver2({x: INIT_X*canvasWidth, y: INIT_Y*canvasHeight})
  ...
  const drawCar1 = (ctx, canvas) => {
    ...
    const newCoords1 = rotate(
      car1Location[frameIndex1 + 1].x - car1Location[frameIndex1].x,
      car1Location[frameIndex1 + 1].y - car1Location[frameIndex1].y,
      SCALE*canvasSize[0],
      ANGLE,
      [-1, 1]
    );
    ...
    ctx.lineTo(driver1.x + newCoords1.x, driver1.y + newCoords1.y); // Draw a line to the ending point
    ctx.stroke(); // Actually draw the line (use ctx.fill() for filled lines)
    ctx.closePath(); // Close the path
    setDriver1((prev) => ({
      x: prev.x + newCoords1.x,
      y: prev.y + newCoords1.y,
    }));
  ...
  const drawCar2 = (ctx, canvas) => {
    ...
    const newCoords2 = rotate(
      ...
      car2Location[frameIndex2 + 1].y - car2Location[frameIndex2].y,
      SCALE*canvasSize[0],
      ANGLE,
      [-1, 1]
    );
    ...
    ctx.lineTo(driver2.x + newCoords2.x, driver2.y + newCoords2.y); // Draw a line to the ending point
    ctx.stroke(); // Actually draw the line (use ctx.fill() for filled lines)
    ctx.closePath(); // Close the path
    setDriver2((prev) => ({
      x: prev.x + newCoords2.x,
      y: prev.y + newCoords2.y,
    }));
